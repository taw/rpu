...
let scene_new () =
	...
	tridata   = DynArray.init 1 (fun _ -> [0.0; 1.0; 0.0]);
...
let scene_load_from_file : string -> (float list, float list) scene =
fun scene_file_name ->
    let scene_file_handler = open_in scene_file_name in
    let scene = scene_new () in
    let (vtx_cnt, tri_cnt) = Scanf.fscanf scene_file_handler "%d %d" (fun a b -> (a,b)) in
    let scan_line_vtx () =
	Scanf.fscanf scene_file_handler "%f %f %f %f %f %f %f %f" (
	    fun x y z nx ny nz s t ->
	    DynArray.add scene.vertices ((v3_make x y z), [nx;ny;nz;s;t])
    )
in
    let scan_line_tri () =
	Scanf.fscanf scene_file_handler "%d %d %d %d" (
	    fun a b c d ->
	    DynArray.add scene.triangles (a,b,c,d)
    )
in
(
    do_times vtx_cnt scan_line_vtx;
    do_times tri_cnt scan_line_tri;
    scene
)
...
let leaf_max     = ref 4
let polygon_file = ref "edgar.raw"
let output_file  = ref "edgar-compiled.dat"
...
let arg_spec = [
    ...
    ("-s", Arg.String (fun arg -> polygon_file := arg), "preprocessed OBJ file");
...
let scene : (float list, float list) scene = scene_load_from_file (!polygon_file) in
