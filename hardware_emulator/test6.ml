...
in
	...
	Vm.vm_setreg vm reg_index_C0 (-0.2) 0.5 0.0 1.0;
	vm.Vm.pc <- 0;
	Vm.vm_run_shader vm;
	pixel (
...
let test_6 : unit -> unit = fun () ->
(
    let vm = Vm.vm_new () in
    vm_load_scene vm "bunny-compiled.dat";
    vm_load_asm vm "phong.shr";
    ...
    let xsz   = 400 in
    let ysz   = 400 in
    render_scene vm orig dir0 right up xsz ysz (open_out "test6.pnm") 0;
)
;;
test_6()
