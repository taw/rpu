...
in
    let run_shader x y =
    (
	Vm.vm_setreg vm reg_index_C0 0.0 0.0 0.0 0.0;          (* orig *)
	Vm.vm_setreg vm reg_index_C1 2.0 0.0 (-1.0) 0.0;       (* dir.x = C1 . P *)
	Vm.vm_setreg vm reg_index_C2 0.0 (-2.0) 1.0 0.0;       (* dir.y = C2 . P *)
	Vm.vm_setreg vm reg_index_C3 0.0 0.0 1.0 1.0;          (* dir.z = C3 . P *)
...
let test_7 : unit -> unit = fun () ->
(
    let vm = Vm.vm_new () in
    vm_load_scene vm "bunny-compiled.dat";
    vm_load_asm vm "phong_trace.shr";
    let xsz   = 400 in
    let ysz   = 400 in
    render_scene vm xsz ysz (open_out "test7.pnm") 0;
)
;;
test_7()
