...
;
	...
	mad R0,R4.z,R1,R0           ; compute hit point
	mov R2,R4
	mov R2.z,R5.w                   ; save address register

par    (mov R3,C0                   ; origin of light ray
        add R4,-C0,R0               ; direction of light ray
	mov R8,C1)                  ; store light intensity

	;mul R0,R8,+0.2
	;mov R0,R0 + return or xyzw (>=0 or <1)

par    (dp3_rcp R5.x,R4,C.yxxx ; S.x = 1/R3 dot (1,0,0,0)
	dp3_rcp R5.y,R4,C.xyxx
	dp3_rcp R5.z,R4,C.xxyx)

par    (mov E0,R3 
	mov E1,S
	mov R7,C3.yyzz
	mov E2,C3 + trace push 3 rty 1)
	mov E0,H + call push 3 rty 1

	;mul R0,R8,+0.2
	;mov R0,R0 + return or xyzw (>=0 or <1)

	mov R4,R2
	mov A.zw,R4                 ; restore address register

	ld4x A2,0                   ; load triangle normal
	add R4.w,R4.x,R4.y          ; and interpolate it
	add R4.w,C.y,-R4.w
	mul R5,R4.x,I0
	mad R5,R4.y,I1,R5
	mad R5,R4.w,I2,R5
...
pos:    mad R2,R1.w,R6,C2.y + jmp cont, or xyzw (>=0 or <1)
neg:    mad R2,-R1.w,R6,C2.y 
cont:
	;mov R0,R2
	;+ return or xyzw (>=0 or <1)

	ld I0,A3,1                  ; load color
	...
	add R3.w,R3.w,R3.w       ; 2* TODO: optimize
	...
	mul R1,S.w,R6          ; specular part multiplied with light intensity
...
par    (mov E0,R3                   ; store ray origin 
	...
	; compute diffuse light intensity
	dp3 R1.w,R4,R5 + jmp neg1, w <0
pos1:	mad R2,R1.w,R8,C2.y + jmp cont1, or xyzw (>=0 or <1)
neg1:    mad R2,-R1.w,R8,C2.y 
cont1:	
	...
	mad R6,+0.02,R3,R0        ; go safety away
